...
export default function PropertiesPage() {
  ...
  const [page_numbers, setPageNumbers] = useState<string>("1");
  const [page_sizes, setPageSizes] = useState<string>("10");

  const navigate = useNavigate();

  const { data, isLoading, error } = useUserListing({
    page: parseInt(page_numbers),
    url: "/properties/list?",
    size: parseInt(page_sizes),
  });
  ...
  const filteredProperties = data?.items?.filter(
    (property: PropertyListInterface) => {
      const matchesSearch =
      ...
      const matchesType =
        typeFilter === "all" || property.property_types === typeFilter;
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
          ...
          <Select value={page_numbers} onValueChange={setPageNumbers}>
            <SelectTrigger className="w-32">
              <SelectValue placeholder="Page Numbers" />
            </SelectTrigger>

            <SelectContent>
              {data?.page_numbers?.map((item: string) => (
                <SelectItem
                  key={`page_numbers_${item.toString()}]`}
                  value={item.toString()}
                >
                  {item.toString()}
                </SelectItem>
          ...
          <Select value={page_sizes} onValueChange={setPageSizes}>
            <SelectTrigger className="w-32">
              <SelectValue placeholder="Page Numbers" />
            </SelectTrigger>

            <SelectContent>
              {data?.page_sizes?.map((item: string) => (
                <SelectItem
                  key={`page_sizes_${item.toString()}]`}
                  value={item.toString()}
                >
                  {item.toString()}
                </SelectItem>
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {filteredProperties?.map((property: PropertyListInterface) => {
          ...
          return (
            <Card
              key={property?.id}
              className="overflow-hidden group cursor-pointer hover:shadow-soft transition-shadow"
            >
